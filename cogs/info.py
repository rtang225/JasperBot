...
#Owner
    ...
    async def owner(self, ctx):
        ...
        embed = discord.Embed(title = 'The Owner', description = f'The owner of this server is {ctx.guild.owner.mention}', color = BotConfig.color())
...
#Bot Info
    ...
    async def botinfo(self, ctx):
        await ctx.message.delete()
        birthday = datetime.datetime.strptime('12-13-2020', '%m-%d-%Y')
        age = (datetime.datetime.now() - birthday).days
        embed = discord.Embed(title = 'About Me!', description = 'The bot is coded with Python. The owner of the bot is <@524001661379936268>. I was created and joined this server on December 19, 2020.', color = BotConfig.color())
        embed.add_field(name = 'Age:', value = f'I am {age} days old! Which is {round(age/365, 2)} years old')
        await ctx.send(embed = embed)
...
#Staff
    ...
    async def staff(self, ctx):
        ...
        embed = discord.Embed(title = 'Staff Members', description = 'Meet our Staff Members!', color = BotConfig.color())
...
#Covid Information
    ...
    async def covid(self, ctx):
        ...
        embed.set_footer(text = "In the case that you don't believe COVID is real, you may 'politely' go fuck yourself")
        await ctx.send(embed = embed)
...
#School Status
    ...
    async def schoolstatus(self, ctx):
        await ctx.message.delete()
        embed = discord.Embed(title = 'Ontario Schools Status', description = 'As of April 12th 2021, all Ontario Schools will be closed until further notice', color = BotConfig.color())
        await ctx.send(embed = embed)
...
#Invite Link
    ...
    async def invite(self, ctx):
        await ctx.message.delete()
        embed = discord.Embed(title = 'Permanent Invite Link:', description = 'https://discord.gg/26jGM68MCA', color = BotConfig.color())
        await ctx.send(embed = embed)
...
#User Info
    ...
    async def userinfo(self, ctx, *, member: commands.MemberConverter = None):
        ...
        if member.raw_status == 'online':
            ...
            status = '🟣 Streaming'
        embed = discord.Embed(title = f'User Info For: {member.name}#{member.discriminator}', color = BotConfig.color())
...
#Server Info
    ...
    async def serverinfo(self, ctx):
        ...
        creation = timezone_convert_creation.strftime('%B %d, %Y at %H:%M EST')
        embed = discord.Embed(title = 'Server Info For: ' + ctx.guild.name, color = BotConfig.color())
...
#Role with list of members
    ...
    async def rolelist(self, ctx, *, role: commands.RoleConverter):
        ...
        embed = discord.Embed(title = f'Members in {role.name} ({len(memberlist)}):', description = members, color = BotConfig.color())
        embed.set_thumbnail(url = ctx.guild.icon)
        await ctx.send(embed = embed)
...
#List of all roles
    ...
    async def roles(self, ctx):
        ...
        role = " ".join(rolelist)
        embed = discord.Embed(title = 'Roles List:', description = role, color = BotConfig.color())
...
#Emotes list
    ...
    async def emotes(self, ctx):
        ...
        embed = discord.Embed(title = 'Emotes List:', description = emotes, color = BotConfig.color())
        await ctx.send(embed = embed)
